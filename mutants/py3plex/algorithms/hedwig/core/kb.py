...
class ExperimentKB:
    ...
    def __init__(self, triplets, score_fun, instances_as_leaves=True):
        ...
        if self.target_type == Example.Ranked:
            ...
            logger.debug(f"Class distribution: {str(self.distribution)}")
    ...
    def _build_examples(self, g):
        ...
        self.examples_uris = list(ex_subjects)
        self.uri_to_idx = {}
        ...
        for i, ex_uri in enumerate(self.examples_uris):
            ...
            if score_list:
                ...
                if not score_list:
                    raise Exception(
                        ...
                        + f"instances of {HEDWIG.Example}, with {HEDWIG.score} or {HEDWIG.class_label} provided."
                    )
            ...
            examples.append(
                Example(i, str(ex_uri), score, annotations=annotations, weights=weights)
            )

        if not examples:
            raise Exception(
                "No examples provided! Examples should be "
                + f"instances of {HEDWIG.Example}."
            )
    ...
    def _build_subclassof(self, g):
        ...
        for predicate in g.subjects(
            predicate=RDF.type, object=HEDWIG.GeneralizationPredicate
        ):
        ...
        for predicate in g.subjects(
            predicate=RDF.type, object=HEDWIG.SpecializationPredicate
        ):
            for sub, obj in g.subject_objects(predicate=predicate):
                if user_defined(sub) and user_defined(obj):
        ...
        for ex in self.examples:
            for inst in ex.annotations:
                if self.instances_as_leaves:
                    ...
                    inst_parents += list(
                        g.objects(subject=URIRef(inst), predicate=RDFS.subClassOf)
                    )
    ...
    def _find_roots(self, all_annotations):
        roots = list(
            filter(lambda pred: not self.sub_class_of[pred], self.super_class_of.keys())
        )
        ...
        for annotation in all_annotations:
            if annotation not in self.predicates:
                ...
                logger.debug(
                    f"Adding leaf {annotation} as root, as it is not specified in the ontology"
                )

        logger.debug(f"Detected root nodes: {str(roots)}")
    ...
    def _calc_members_closure(self):
        ...
        def closure(pred, lvl, visited=None):

            if visited is None:
                visited = []
            if pred in visited:
                raise Exception(f"Cycle detected in the hierarchy at predicate {pred}!")
    ...
    def _propagate_annotation_names(self, g):
        ...
        for sub, obj in g.subject_objects(predicate=HEDWIG.annotation_name):
            ...
            logger.debug(f"Annotation name root: {sub}, {obj}")
    ...
    def get_root(self):
        ...
        return UnaryPredicate(
            self.dummy_root, self.get_full_domain(), self, custom_var_name="X"
        )
