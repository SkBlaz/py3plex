...
class HeterogeneousInformationNetwork:
    ...
    def add_label(self, node, label_id, label_name=None):
        ...
        if label_id in self.labels_by_id:
            if self.labels_by_id[label_id] not in self.graph.nodes[node]["labels"]:
                self.graph.nodes[node]["labels"].append(self.labels_by_id[label_id])
    ...
    def process_network(self, label_delimiter):
        if self.target_tag:
            basic_types = {
                self.graph.nodes[x]["type"]
                for x in self.graph.nodes
                if "labels" in self.graph.nodes[x]
            }

            print(f"Target type: {basic_types}")
    ...
    def create_label_matrix(self, weights=None):
        ...
        for i, label in enumerate(self.label_list):
            ...
            if weights == "balanced":
                self.label_matrix[member_indices, i] = 1.0 / max(
                    len(label.train_indices), 1
                )
            else:
    ...
    def calculate_decomposition_candidates(self, max_decomposition_length=10):
        ...
        candidate_lists = []
        for _i in range(max_decomposition_length - 1):
            next_gens = []
            for list_so_far in under_construction:
                if (
                    list_so_far["node_list"][-1] != self.basic_type
                    or len(list_so_far["node_list"]) == 1
                ):
                            ...
                            append_to.append(
                                {
                                    ...
                                    "edge_list": list_so_far["edge_list"]
    ...
    def split_to_indices(self, train_indices=(), validate_indices=(), test_indices=()):
        ...
        self.train_ids = {self.node_list[i] for i in self.train_indices}
        self.validate_ids = {self.node_list[i] for i in self.validate_indices}
        self.test_ids = {self.node_list[i] for i in self.test_indices}
    ...
    def decompose_from_iterator(
        ...
        classes = [
            lab for lab in self.label_list if lab and len(lab.not_test_members) > 0
        ]
        ...
        for i, label in enumerate(classes):
            label_matrix[
                [universal_inv[item] for item in label.not_test_members], i
            ] = 1
        ...
        if parallel:
            ...
            while True:
                tmp_container = [next(generator) for _ in range(bsize)]
            ...
            for item in generator:
                ...
                importances = importance_calculator(
                    classes,
                    universal_set,
                    item,
                    n,
