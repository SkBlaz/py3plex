...
def aggregate_weighted_sum(input_thing, classes, universal_set):
    n = len(universal_set)
    weights = [
        (len(cl.train_members) + len(cl.validate_members)) * 1.0 / n for cl in classes
    ]
...
def calculate_importance_okapi(
    ...
    for i, linked_node in enumerate(linked_nodes):
        return_vec[i] = (k1 + 1) / (
            1 + k1 * (1 - b + b * degrees[linked_node] / avgdegree)
        )
...
def calculate_importance_delta(classes, universal_set, linked_nodes, n, **kwargs):
    ...
    for label in classes:
        ...
        diff = actual_pos_num * 1.0 / (predicted_pos_num + 1) - actual_neg_num * 1.0 / (
            predicted_neg_num + 1
        )
...
def gr_value(actual_pos_num, predicted_pos_num, tp, n):
    ...
    if pp == 1 or pp == 0:
        return 0
    return ig_value(actual_pos_num, predicted_pos_num, tp, n) / (
        -pp * log(pp, 2) - (1 - pp) * log((1 - pp), 2)
    )
