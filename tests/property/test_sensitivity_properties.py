        ...
        for i in range(len(nodes) - 1):
            for j in range(i + 1, len(nodes)):
                if np.random.random() > 0.5:  # 50% edge probability
                    network.add_edges(
                        [[nodes[i], layer, nodes[j], layer, 1.0]], input_type="list"
                    )
...
class TestStabilityMetricsProperties(unittest.TestCase):
    ...
    def test_jaccard_reflexive(self):
        ...
        for k in range(1, len(ranking) + 1):
            ...
            self.assertAlmostEqual(
                jaccard, 1.0, places=5, msg=f"Jaccard(X, X) != 1.0 for k={k}"
            )
    ...
    def test_jaccard_symmetric(self):
        ...
        for k in range(1, len(baseline) + 1):
            ...
            self.assertAlmostEqual(
                jaccard_xy, jaccard_yx, places=5, msg=f"Jaccard not symmetric for k={k}"
            )
    ...
    def test_jaccard_range(self):
        ...
        for k in range(1, len(baseline) + 1):
            ...
            self.assertGreaterEqual(jaccard, 0.0, msg=f"Jaccard < 0 for k={k}")
            self.assertLessEqual(jaccard, 1.0, msg=f"Jaccard > 1 for k={k}")
...
class TestPerturbationProperties(unittest.TestCase):
    ...
    def test_edge_drop_preserves_nodes(self):
        ...
        self.assertEqual(
            original_nodes, perturbed_nodes, msg="Edge dropping changed node set"
        )
    ...
    def test_edge_drop_deterministic(self):
        ...
        self.assertEqual(
            len(edges1), len(edges2), msg="Same seed produced different edge counts"
        )
...
class TestSensitivityDistinctFromUQ(unittest.TestCase):
    ...
    def test_sensitivity_not_mean_std_ci(self):
        ...
        self.assertIsInstance(
            jaccard, (float, np.floating), msg="Stability metric returned wrong type"
        )
...
class TestSensitivityCurveProperties(unittest.TestCase):
    ...
    def test_increasing_perturbation_nonincreasing_stability(self):
        ...
        self.assertLessEqual(
            jaccard2, jaccard1, msg="Severe perturbation has higher stability than mild"
        )
