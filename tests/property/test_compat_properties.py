...
from py3plex.compat import convert, to_ir, from_ir
...
class TestNetworkXRoundtripProperties:
    ...
    def test_networkx_conversion_preserves_reserved_attribute_names(self):
        """Regression: attributes named like NetworkX method parameters survive."""
        graph = MultiLayerGraph(directed=True)
        graph.add_layer(Layer(id="L1"))
        graph.add_node(Node(id=0, attributes={"self": "node-ok"}))
        graph.add_node(Node(id=1, attributes={}))
        graph.add_edge(
            Edge(
                src=0,
                dst=1,
                src_layer="L1",
                dst_layer="L1",
                attributes={"self": "edge-ok", "weight": 1.0},
            )
        )
        ...
        assert nx_graph.nodes[0]["self"] == "node-ok"
        assert nx_graph.edges[0, 1]["self"] == "edge-ok"
        assert restored.nodes[0].attributes["self"] == "node-ok"
        assert restored.edges[0].attributes["self"] == "edge-ok"

    def test_networkx_import_does_not_mutate_input_attributes(self):
        """Regression: converting from NetworkX leaves caller-owned attrs intact."""
        graph = nx.MultiDiGraph()
        graph.add_node("A", _py3plex_node_order=0, keep="node")
        graph.add_node("B", _py3plex_node_order=1)
        graph.add_edge(
            "A",
            "B",
            key=7,
            _py3plex_edge_id="edge-7",
            _py3plex_edge_order=0,
            _py3plex_key=7,
            keep="edge",
        )

        convert(graph, "multilayer_graph")

        assert graph.nodes["A"]["_py3plex_node_order"] == 0
        assert graph.edges["A", "B", 7]["_py3plex_edge_id"] == "edge-7"
        assert graph.edges["A", "B", 7]["_py3plex_key"] == 7
