...
class TestIncidenceGadgetEncoding:
    ...
    def test_node_mapping_correctness(self):
        ...
        network = multinet.multi_layer_network(directed=False)
        network.add_nodes([
            {'source': 'node1', 'type': 'L1'},
            {'source': 'node2', 'type': 'L1'}
        ], input_type='dict')
        network.add_edges([
            {'source': 'node1', 'target': 'node2', 'source_type': 'L1', 'target_type': 'L1'}
        ], input_type='dict')
    ...
    def test_decoding_without_encoding(self):
        ...
        for layer, edges in recovered.items():
            assert "layer_with_prime_" in layer, f"Expected layer name with prime, got: {layer}"
    ...
    def test_large_network(self):
        ...
        edges_l1 = [{'source': str(i), 'target': str(i + 1), 'source_type': 'L1', 'target_type': 'L1'} for i in range(9)]
        edges_l2 = [{'source': str(i), 'target': str(i + 1), 'source_type': 'L2', 'target_type': 'L2'} for i in range(9)]
    ...
    def test_edge_info_structure(self):
        ...
        network.add_edges([
            {'source': 'X', 'target': 'Y', 'source_type': 'TestLayer', 'target_type': 'TestLayer'}
        ], input_type='dict')
