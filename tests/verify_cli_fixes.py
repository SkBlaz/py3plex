...
def analyze_cli_code():
    ...
    if "def _get_layer_names(" in code:
        checks_passed.append("✓ _get_layer_names helper function exists")
    else:
        checks_failed.append("✗ _get_layer_names helper function not found")
    
    # Check 2: _get_layer_names is used in cmd_load
    if "_get_layer_names(network)" in code and "def cmd_load(" in code:
        checks_passed.append("✓ cmd_load uses _get_layer_names")
    else:
        checks_failed.append("✗ cmd_load doesn't use _get_layer_names")
    ...
    if "def cmd_stats(" in code:
        ...
        if get_layer_names_usage > cmd_stats_pos and (next_function_pos == -1 or get_layer_names_usage < next_function_pos):
            checks_passed.append("✓ cmd_stats uses _get_layer_names")
        else:
            checks_failed.append("✗ cmd_stats doesn't use _get_layer_names")
    else:
        checks_failed.append("✗ cmd_stats function not found")
    ...
    if "network.get_layers(" in code and "layer_names, layer_graphs, multiedges" in code:
        checks_passed.append("✓ cmd_visualize properly unpacks get_layers tuple")
    else:
        checks_failed.append("✗ cmd_visualize doesn't properly unpack get_layers")
    ...
    if "list(layer_graphs.values())" in code or "layer_graphs.values()" in code:
        checks_passed.append("✓ cmd_visualize passes graph list to draw function")
    else:
        checks_failed.append("✗ cmd_visualize might pass wrong type to draw function")
    
    # Check 6: Documentation includes edgelist examples
    if "edgelist" in code.lower() and "examples:" in code.lower():
        checks_passed.append("✓ Documentation includes edgelist format examples")
    else:
        checks_failed.append("✗ Documentation lacks edgelist examples")
    ...
    if problematic_pattern not in code:
        checks_passed.append("✓ No direct get_layers()[0] indexing found")
    else:
        # This might be okay in some contexts, so just warn
        checks_passed.append("⚠ get_layers()[0] found - verify it's used correctly")
    ...
    for func_name, expected_args in functions_to_check.items():
        if func_name in found_functions:
            ...
            if actual_args == expected_args:
                ...
                print(f"  ✗ {func_name}: expected {expected_args}, got {actual_args}")
                all_good = False
        else:
            if func_name == '_get_layer_names':
                # This is a new function, so it's okay if it doesn't exist yet
                print(f"  ⚠ {func_name} not found (might be new)")
            else:
