...
1. **Infection probability** for susceptible node i:
   ```
   λ_{i,α}(t) = β_α w_α Σ_j A^(α)_{ji} 𝟙{X_j = I}
   p_i(t) = 1 - ∏_α exp(-λ_{i,α}(t) dt)
   ```

2. **Recovery probability** for infected node i:
   ```
   p_rec(i) = 1 - exp(-γ_i dt)
   ```
...
1. Compute total event rate:
   ```
   Λ = Σ_{i∈S} Σ_α λ_{i,α} + Σ_{i∈I} γ_i + η|S|
   ```
...
5. Record event and repeat until t > t_max or no infected remain
...
- Consider vectorized operations are used internally for efficiency

## Examples

See `examples/sir_notebook.ipynb` for additional examples including:
