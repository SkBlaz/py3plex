...
def visualize_multilayer_ricci_core(
    ...
    **kwargs
    ...
    node_colors = _compute_node_colors(
        net, G, node_color_by, curvature_attr
    )
    ...
    if dim == 2:
        _draw_2d_network(
            G, positions, node_colors, node_sizes, edge_colors, edge_widths,
            ax, cmap_nodes, cmap_edges, show_colorbar
        )
    else:
        _draw_3d_network(
            G, positions, node_colors, node_sizes, edge_colors, edge_widths,
            ax, cmap_nodes, cmap_edges
        )
    
    ax.set_title(f"Ricci Flow Visualization - Core Network\n"
                 f"(α={alpha}, iterations={iterations})", fontsize=12)
    ax.axis('off')
...
def _compute_node_colors(net, G, color_by, curvature_attr):
    ...
    if color_by == "layer_overlap":
        ...
        colors = []
        for node in nodes:
            count = sum(1 for layer_name, layer_graph in net.layer_names.items()
...
def _draw_2d_network(G, positions, node_colors, node_sizes, edge_colors, edge_widths,
                     ax, cmap_nodes, cmap_edges, show_colorbar):
...
def _draw_3d_network(G, positions, node_colors, node_sizes, edge_colors, edge_widths,
                     ax, cmap_nodes, cmap_edges):
...
def visualize_multilayer_ricci_layers(
    ...
    **kwargs
    ...
    if share_layout:
        ...
        if compute_if_missing:
            net.compute_ollivier_ricci_flow(
                mode="layers", layers=layers, alpha=alpha,
    ...
    for idx, layer_id in enumerate(layers):
        ...
        ax_current = axes[idx]
        _draw_single_layer(
            layer_graph, layer_pos, layer_id, ax_current,
            node_size, edge_width_scale, weight_attr, curvature_attr, cmap_edges
        )
    ...
    fig.suptitle(f"Ricci Flow Per-Layer Visualization\n"
...
def _draw_single_layer(layer_graph, positions, layer_id, ax, node_size,
                       edge_width_scale, weight_attr, curvature_attr, cmap_edges):
    """Draw a single layer on the given axes."""
    # Compute edge colors and widths
    edge_colors = [layer_graph[u][v].get(curvature_attr, 0) for u, v in layer_graph.edges()]
    edge_widths = _compute_edge_widths(layer_graph, weight_attr, edge_width_scale)
    
    # Draw edges
    if layer_graph.number_of_edges() > 0:
        nx.draw_networkx_edges(
            layer_graph, positions, ax=ax,
            width=edge_widths,
            edge_color=edge_colors,
            edge_cmap=plt.get_cmap(cmap_edges),
            alpha=0.6
...
def visualize_multilayer_ricci_supra(
    ...
    **kwargs
    ...
    if compute_if_missing:
        ...
        has_curv = any(curvature_attr in data for _, _, data in G_supra.edges(data=True))
        if not has_curv:
            raise ValueError(
    ...
    if node_color_by == "layer":
        ...
        node_colors = _compute_edge_colors(G_supra, edge_color_by, curvature_attr, weight_attr)
    
    edge_colors_intra = [
        G_supra[u][v].get(curvature_attr, 0) for u, v in intra_edges
    ]
    edge_colors_inter = [
        G_supra[u][v].get(curvature_attr, 0) for u, v in inter_edges
    ]
    
    # Draw
    if dim == 2:
        _draw_supra_2d(
            G_supra, positions, intra_edges, inter_edges,
            node_colors, edge_colors_intra, edge_colors_inter,
            node_size, edge_width_scale, weight_attr, ax,
            cmap_nodes, cmap_edges, interlayer_alpha
        )
    else:
        _draw_supra_3d(
            G_supra, positions, intra_edges, inter_edges,
            node_colors, node_size, ax, interlayer_alpha
        )
    
    ax.set_title(f"Ricci Flow Supra-Graph Visualization\n"
                 f"(α={alpha}, iterations={iterations})", fontsize=12)
    ax.axis('off')
...
def _draw_supra_2d(G_supra, positions, intra_edges, inter_edges,
                   node_colors, edge_colors_intra, edge_colors_inter,
                   node_size, edge_width_scale, weight_attr, ax,
                   cmap_nodes, cmap_edges, interlayer_alpha):
    """Draw 2D supra-graph."""
    ...
    if inter_edges:
        ...
        nx.draw_networkx_edges(
            G_supra, positions, edgelist=inter_edges, ax=ax,
            width=inter_widths,
            edge_color='gray',
    ...
    if intra_edges:
        ...
        nx.draw_networkx_edges(
            G_supra, positions, edgelist=intra_edges, ax=ax,
            width=intra_widths,
...
def _draw_supra_3d(G_supra, positions, intra_edges, inter_edges,
                   node_colors, node_size, ax, interlayer_alpha):
    """Draw 3D supra-graph."""
    # Draw nodes
    ...
    for u, v in inter_edges:
        ax.plot(
            ...
            [positions[u][1], positions[v][1]],
            [positions[u][2], positions[v][2]],
            color='gray', linewidth=0.5, alpha=interlayer_alpha, linestyle='--'
        )
