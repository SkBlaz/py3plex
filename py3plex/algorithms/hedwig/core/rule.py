...
class Rule:
    ...
    def __init__(self, kb, predicates=None, target=None):
    ...
    def _replace_predicate(self, target, replacement):
        ...
        self.covered_examples = (
            self.covered_examples & replacement.domain[replacement.input_var]
        )
    ...
    def _plain_report(self, show_uris=False, human=lambda label, rule: label):
        ...
        """
        s = (
            self._plain_conjunctions(show_uris=show_uris, human=human)
            + " "
    ...
    def _plain_conjunctions(self, show_uris=False, human=lambda label, rule: label):
        ...
        for pred in self.predicates:
            ...
            if isinstance(pred, UnaryPredicate):
                ...
                conj = f"{predicate_label}({pred.input_var}, {label})"
            else:
                conj = f"{label}({pred.input_var}, {pred.output_var})"
    ...
    @staticmethod
    def ruleset_report(
        rules, show_uris=False, latex=False, human=lambda label, rule: label
    ):
        if latex:
        ...
        for target_class, rules in rules_per_target:
            ...
            for _, rule in enumerate(
                ...
                class_examples.append(
                    (rule._plain_conjunctions(), [ex.label for ex in examples])
                )
