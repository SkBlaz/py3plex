    ...
    for path in paths:
        if path.endswith(def_format):
            try:
                ...
                errorMsg = (
                    errorMsg + "Error parsing file: " + path + ".\n" + str(e) + "\n\n"
                )
                errorCount += 1
    if errorCount > 0:
...
def build_uri(class_string):
    ...
    if class_string.startswith("http://"):
        ...
        class_uri = rdflib.term.URIRef(f"{str(GENERIC_NAMESPACE)}{class_string}")
...
def csv_parse_data(g, data_file):
    ...
    with open(data_file) as f:
        ...
        logger.debug(f"Attributes: {str(attributes)}")
    ...
    for example in examples:
        ...
        for att_idx, att in enumerate(attributes):
            ...
            if not (value_is_uri or attribute_value == "1"):
                continue
            annotation_uri = (
                build_uri(attribute_value) if value_is_uri else build_uri(att)
            )
            blank = rdflib.BNode()
