...
class SuccessiveHalvingConfig:
    ...
    def __post_init__(self):
        ...
        if self.utility_method not in ("mean_minus_std", "expected_regret", "prob_near_best"):
            raise ValueError(
...
class SuccessiveHalvingRacer:
    ...
    def race(
        ...
        if n_rounds is None:
            n_rounds = math.ceil(math.log(len(algorithm_ids)) / math.log(self.config.eta))
            ...
            for metric_name in metric_names:
                try:
                    if metric_name == "modularity":
                        ...
                        non_singleton = sum(
                            1 for node, comm in result.partition.items()
                            if comm_sizes[comm] > 1
                        )
                        value = non_singleton / len(result.partition) if result.partition else 0.0
                    
                    elif metric_name == "stability":
                        # Placeholder: would need UQ data
                        value = 0.5
    ...
    def _select_survivors(
        ...
        sorted_algos = sorted(
            survivors,
            key=lambda a: utilities.get(a, -float('inf')),
            reverse=True
        )
