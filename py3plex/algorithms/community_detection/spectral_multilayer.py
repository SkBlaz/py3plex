...
def spectral_multilayer_supra(
    ...
    unique_nodes = sorted(set(node for node, layer in nodes_list))
    unique_layers = sorted(set(layer for node, layer in nodes_list))
    n = len(unique_nodes)
    L = len(unique_layers)
    ...
    node_layer_to_idx = {(node, layer): i for i, (node, layer) in enumerate(nodes_list)}
