...
class MultiXRank:
    ...
    def add_multiplex(
        ...
        if self.verbose:
            logger.info(f"Added multiplex '{name}' with dimension {supra_adjacency.shape[0]}")
    ...
    def build_supra_heterogeneous_matrix(self, block_weights: Optional[Dict[str, float]] = None):
        """
        ...
        if self.verbose:
            logger.info(f"Building supra-heterogeneous matrix of size {total_dim}×{total_dim}")
        ...
        self.supra_matrix = sp.csr_matrix((data, (rows, cols)), shape=(total_dim, total_dim))
        
        if self.verbose:
            logger.info(f"Built supra-heterogeneous matrix with {self.supra_matrix.nnz} edges")
    ...
    def random_walk_with_restart(
        ...
        for iteration in range(self.max_iter):
            p_new = (1 - self.restart_prob) * (self.transition_matrix @ p) + self.restart_prob * seed_vec
            ...
            if diff < self.epsilon:
                converged = True
                if self.verbose:
                    logger.info(f"RWR converged after {iteration + 1} iterations (L1 diff: {diff:.2e})")
                break
...
def multixrank_from_py3plex_networks(
    ...
    bipartite_connections: Optional[Dict[Tuple[str, str], Union[sp.spmatrix, np.ndarray]]] = None,
    ...
    for name, network in networks.items():
        ...
        node_order = network.node_order_in_matrix if hasattr(network, 'node_order_in_matrix') else None
