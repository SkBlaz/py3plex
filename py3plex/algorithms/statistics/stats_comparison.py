...
def compare_multilayer_networks(
    ...
    if test not in valid_tests:
        raise ValueError(f"Invalid test '{test}'. Must be one of: {', '.join(valid_tests)}")
    ...
    metric_values = _extract_metrics(
        networks, metrics, layer_scope, custom_metrics
    )
    ...
    if len(df) == 0:
        return pd.DataFrame(columns=[
            'metric', 'layer', 'statistic', 'p_value',
            'adjusted_p_value', 'effect_size', 'significant'
        ])
...
def _compute_coupling_strength(network: Any, layers: List[str]) -> Dict[str, List[float]]:
...
def _compute_entropy(network: Any) -> Dict[str, List[float]]:
    ...
    try:
        ...
        warnings.warn(f"Could not compute entropy: {e}")
        return {'global': [0.0]}
...
def _perform_test(
    ...
    try:
        ...
        # Compute effect size
        if n_groups == 2:
            effect_size = _compute_cohens_d(values_per_network[0], values_per_network[1])
        else:
        ...
        warnings.warn(f"Test failed for {metric_name}/{layer}: {e}")
