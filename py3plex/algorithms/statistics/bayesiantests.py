...
def hierarchical_MC(
    ...
    for i in range(0, len(x)):
        if np.std(x[i, :]) == 0:
            x[i, :] = x[i, :] + np.random.normal(
                0, np.min(1 / 1000000000, np.abs(np.mean(x[i, :]) / 100000000))
            )
    ...
    */
    ...
    # Call to Stan code
    fit = pystan.stan(
        model_code=hierarchical_code, data=hierachical_dat, iter=1000, chains=4
    )
...
def plot_simplex(points, names=("C1", "C2"), proba_triplet=None):
    ...
    for i, n in enumerate((nl, ne, nr)):
        if n < 0.001:
            print(
                "p{} is too small, switching to 2d plot".format(names[::-1] + ["rope"])
            )
    ...
    for i in (1, 2, 3):
        fig.gca().add_line(
            Line2D(
                [vert0[0, 0], vert0[i, 0]], [vert0[0, 1], vert0[i, 1]], color="black"
            )
        )
    # vertex labels
    rcParams.update({"font.size": 16})
    fig.gca().text(-0.08, -0.08, f"p({names[0]} ({proba_triplet[0]}))", color="black")
    fig.gca().text(0.44, np.sqrt(3) / 2 + 0.05, "p(rope)", color="black")
    fig.gca().text(0.650, -0.08, f"p({names[1]} ({proba_triplet[2]}))", color="black")
