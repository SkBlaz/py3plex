...
def inter_layer_coupling_strength(
    network: Any, 
    layer_i: str, 
    layer_j: str
) -> float:
    ...
    for edge in network.get_edges(data=True):
        ...
        if ((l1 == layer_i and l2 == layer_j) or 
            (l1 == layer_j and l2 == layer_i)):
...
def inter_layer_degree_correlation(
    network: Any,
    layer_i: str,
    layer_j: str
) -> float:
...
def versatility_centrality(
    ...
    for layer in all_layers:
        ...
        try:
            ...
            layer_centralities[layer] = {node: cent.get(node, 0.0) for node in all_nodes}
        except:
            layer_centralities[layer] = {node: 0.0 for node in all_nodes}
    ...
    for node in all_nodes:
        v = sum(alpha.get(layer, 0.0) * layer_centralities[layer][node] 
                for layer in all_layers)
...
def multilayer_motif_frequency(
    network: Any,
    motif_size: int = 3
) -> Dict[str, float]:
    """
...
def resilience(
    ...
    if perturbation_type == 'layer_removal' and perturbation_param is not None:
        ...
        nodes_to_remove = [
            (n, l) for n, l in perturbed_graph.nodes() 
            if l == perturbation_param
        ]
        ...
        inter_layer_edges = [
            (n1, n2) for n1, n2 in perturbed_graph.edges()
