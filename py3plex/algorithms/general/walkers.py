...
def basic_random_walk(
    ...
    for _ in range(walk_length):
        ...
        if weighted and G.is_multigraph():
            ...
            weights = np.array([G[current][neighbor].get('weight', 1.0) for neighbor in neighbors])
        else:
...
def node2vec_walk(
    ...
    if weighted:
        weights = np.array([G[start_node][neighbor].get('weight', 1.0) for neighbor in neighbors])
    else:
        weights = np.ones(len(neighbors))
...
def generate_walks(
    ...
    seed: Optional[int] = None
) -> Union[List[List[Union[int, str]]], List[List[Tuple[Union[int, str], Union[int, str]]]]]:
    """
