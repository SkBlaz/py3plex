...
def parse_claim(claim_str: str, params: Optional[Dict[str, Any]] = None) -> Tuple[str, Callable, Callable]:
...
def _parse_predicate(pred_str: str, params: Dict[str, Any], is_antecedent: bool) -> Callable:
    ...
    if not match:
        raise ClaimParseError(
            ...
            suggestions=["Use format: metric__comparator(param) or metric__rank_comparator(param)"],
        )
    ...
    if is_antecedent:
        ...
        if is_rank:
            ...
            def consequent_rank_pred(node_data: Dict[str, Any], metrics: Dict[str, Any]) -> bool:
                if metric not in node_data:
                    return False
            ...
            def consequent_value_pred(node_data: Dict[str, Any], metrics: Dict[str, Any]) -> bool:
                if metric not in node_data:
                    return False
                value = node_data[metric]
                return _compare(value, op_str, param_value)
