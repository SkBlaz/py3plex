...
class PerformanceMonitor:
    ...
    def record(self, func_name: str, elapsed: float, memory_delta: Optional[float] = None):
        """
    ...
    def get_report(self) -> str:
        ...
        lines.append(f"{'Function':<40} {'Calls':>8} {'Total(s)':>10} {'Avg(ms)':>10} {'Min(ms)':>10} {'Max(ms)':>10}")
        lines.append("-" * 80)
        
        for func_name, stats in sorted(self.stats.items()):
            avg_time = (stats['total_time'] / stats['call_count']) * 1000  # Convert to ms
            ...
            try:
                ...
                if memory_delta is not None:
                    if abs(memory_delta) > 1.0:
                        ...
                        logger.debug(f"{func_name} memory delta: {memory_delta*1024:+.2f}KB")
...
def benchmark(
    ...
    return {
        'mean': sum(times) / n,
        'median': times_sorted[n // 2] if n % 2 == 1 else (times_sorted[n // 2 - 1] + times_sorted[n // 2]) / 2,
        'min': min(times),
        'max': max(times),
