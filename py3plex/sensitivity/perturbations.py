    ...
    if layer_aware and hasattr(perturbed_net, 'get_layers'):
        ...
        for layer in layers:
            ...
            for edge_tuple in perturbed_net.get_edges(data=True):
                if len(edge_tuple) >= 3:
                    src_info, dst_info, data = edge_tuple[0], edge_tuple[1], edge_tuple[2] if len(edge_tuple) > 2 else {}
            ...
            if n_to_rewire > 0 and len(layer_edges) >= 2:
                ...
                for src, dst, data in edges_to_rewire:
                    ...
                    for _ in range(max_attempts):
                        ...
                        if (new_edge1 not in [(e[0], e[1]) for e in layer_edges] and
                            new_edge2 not in [(e[0], e[1]) for e in layer_edges] and
                            src != other_dst and other_src != dst):
                            # Perform rewiring
                            try:
                                # Remove old edges
                                perturbed_net.remove_edge((src, layer, dst, layer))
                                perturbed_net.remove_edge((other_src, layer, other_dst, layer))
                                
                                # Add new edges
                                perturbed_net.add_edge(src, other_dst, layer, layer, data.get('weight', 1.0))
                                perturbed_net.add_edge(other_src, dst, layer, layer, other_data.get('weight', 1.0))
                                break
                            except Exception:
