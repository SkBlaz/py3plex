...
def run_sensitivity_analysis(
    ...
    **kwargs
    ...
    logger.info(f"Running sensitivity analysis with {len(grid)} grid points, {n_samples} samples each...")
    
    for p_idx, perturbation_strength in enumerate(grid):
        logger.debug(f"Grid point {p_idx+1}/{len(grid)}: strength={perturbation_strength}")
    ...
    for metric_spec, curve_data in curves.items():
        stability_curves[metric_spec] = StabilityCurve(
            ...
            std=curve_data["std"],
            collapse_point=_find_collapse_point(curve_data["values"], curve_data["grid"]),
        )
...
def _find_collapse_point(values: List[float], grid: List[float], threshold: float = 0.5) -> Optional[float]:
