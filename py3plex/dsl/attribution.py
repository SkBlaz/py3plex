...
class AttributionConfig:
    ...
    edge_scope: str = "incident"  # "incident", "ego_k_hop", "shortest_path_sample", "global_top_m"
    k_hop: int = 2
    max_edges: int = 40
    top_k_layers: int = 10
    top_k_edges: int = 20
    ...
    def validate(self) -> List[str]:
        ...
        if self.objective not in ["value", "rank"]:
            warnings.append(f"Invalid objective '{self.objective}', must be 'value' or 'rank'")
        
        if self.method not in ["shapley", "shapley_mc", "influence"]:
            warnings.append(f"Invalid method '{self.method}', must be 'shapley', 'shapley_mc', or 'influence'")
        
        if self.n_permutations < 16:
            warnings.append(f"n_permutations ({self.n_permutations}) too low, minimum is 16")
        
        if "edge" in self.levels and self.edge_scope not in ["incident", "ego_k_hop", "shortest_path_sample", "global_top_m"]:
            warnings.append(f"Invalid edge_scope '{self.edge_scope}'")
        
        if self.uq not in ["off", "propagate", "summarize_only"]:
            warnings.append(f"Invalid uq mode '{self.uq}', must be 'off', 'propagate', or 'summarize_only'")
...
class AttributionEngine:
    ...
    def compute_attribution(
        ...
        if "edge" in self.config.levels:
            edge_contrib, candidate_edge_count, used_edge_count, edge_scope = (
                self._compute_edge_attribution(item, metric_values, full_value, warnings)
            )
        ...
        return AttributionResult(
            ...
            n_permutations=self.config.n_permutations if self.config.method == "shapley_mc" else None,
    ...
    def _compute_rank_utility(
        ...
        if limit and limit < len(sorted_items):
            ...
            cutoff_value = sorted_items[limit][1] if limit < len(sorted_items) else sorted_items[-1][1]
    ...
    def _compute_layer_attribution(
        ...
        if use_exact:
            ...
            # Monte Carlo Shapley
            if self.config.method == "shapley" and n_features > self.config.max_exact_features:
                warnings.append(
                ...
                if node_key in G:
                    for neighbor in G.neighbors(node_key):
                        ...
                        if edge_data:
                            ...
                            layer_degrees[edge_layer] = layer_degrees.get(edge_layer, 0) + 1
    ...
    def _get_candidate_edges(
        ...
        if scope != "incident":
            warnings.append(f"Edge scope '{scope}' not fully implemented, using 'incident'")
            scope = "incident"
        ...
        if hasattr(self.network, "core_network"):
            ...
            if node_key in G:
                for neighbor in G.neighbors(node_key):
                    ...
                    edge = (node_id, neighbor[0] if isinstance(neighbor, tuple) else neighbor,
                           layer or edge_layer, edge_layer)
                    edges.append(edge)
    ...
    def _compute_mc_shapley_edges(
        ...
        for perm_idx in range(self.config.n_permutations):
            ...
            for pos in perm:
                ...
                curr_value = float(len(subset)) if self.config.metric == "degree" else prev_value
...
def compute_attribution_for_rows(
    ...
    if validation_warnings:
        ...
        if errors:
            raise Py3plexException(f"Attribution configuration errors: {'; '.join(errors)}")
