...
The multilayer modularity $Q$ is defined as:
...
where:
- $A^{[\alpha]}_{ij}$ is the adjacency matrix of layer $\alpha$
- $P^{[\alpha]}_{ij}$ is the expected weight under a null model (e.g., $k_i^\alpha k_j^\alpha / 2m_\alpha$)
- $\gamma^{[\alpha]}$ is the resolution parameter for layer $\alpha$
- $\omega_{\alpha\beta}$ is the inter-layer coupling strength
...
A multilayer network can be represented as a supra-adjacency matrix of size $NL \times NL$ (for $N$ nodes and $L$ layers), where:
- Diagonal blocks contain the intra-layer adjacency matrices $A^{[\alpha]}$
- Off-diagonal blocks encode inter-layer connections (e.g., $\omega I$ for identity coupling)
...
The coupling parameter $\omega$ controls how much communities align across layers:
...
- [Multilayer Centrality Tutorial](multilayer_centrality_tutorial.md)
- [Supra-Adjacency Matrices](../docfiles/supra.rst)
