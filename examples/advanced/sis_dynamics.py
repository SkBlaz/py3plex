...
def run_sis_example() -> SISDynamics:
    ...
    print(f"\nSIS Parameters:")
    print(f"  beta (beta): 0.3")
    print(f"  gamma (gamma): 0.1")
    print(f"  Initial infected: 5%")
    print(f"  R0 estimate: beta/gamma * <k> = {0.3/0.1 * 6:.2f}")
...
def analyze_endemic_threshold() -> None:
    ...
    # Theory: endemic state exists when beta/gamma > 1/<k>
    # Or equivalently: R0 = beta/gamma * <k> > 1
    ...
    plt.axvline(1.0, color='red', linestyle='--', linewidth=2, label='Epidemic threshold (R0=1)')
    plt.axhline(0.0, color='gray', linestyle=':', alpha=0.5)
    plt.xlabel('R0 (basic reproduction number)', fontsize=12)
    ...
    for beta, R0, prev in zip(beta_values, R0_values, final_prevalence):
        ...
        print(f"  beta={beta:.2f}, R0={R0:.2f}: prevalence={prev:.2%} ({status})")
