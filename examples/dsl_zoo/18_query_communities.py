# Query communities via Q.communities
...
Q.nodes().from_layers(L["*"]).community(method="leiden", gamma=1.0, omega=1.0, random_state=42, partition_name="default").execute(net)

# Then query them
result = (
    Q.communities(partition="default")
    .summarise(size="n()", n_layers="n_unique(layer)")
    .execute(net)
)
